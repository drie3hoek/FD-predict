// File:    predict.cpp
//
// Purpose: predict to which Genus (Archips or Clepsis) a photo of a
// 	    	leafroller belongs.
...
{
    cerr << "Usage: " << name << " [<options>] model image..." << endl;
...
fdeep::tensor cimg_to_tensor(const cimg_library::CImg<unsigned char>& image, float mean[]) {
    ...
    for (int y = 0; y < height; y++) {
       for (int x = 0; x < width; x++) {
          for (int c = 0; c < 3; c++) {
	         // Subtract mean values from cfg file
...
int main(int argc, char* argv[]) {
	...
	if (images.size() == 0) {
		cerr << "No images given" << endl << endl;
        show_usage(myname);
		return(1);
	}
	for (vector<string>::iterator it = images.begin(); it != images.end(); it++) {
		...
		// Get the extension and check if this is a jpg or png file
        string extension = getFileExtension(*it);				
	    if (strcmp(extension.c_str(),".jpg") != 0 && strcmp(extension.c_str(),".jpeg") != 0
		 && strcmp(extension.c_str(), ".png") != 0) {
            cerr << "File " << *it << " no .jpg, .jpeg or .png image\n\n";
			return(1);
	    }
	}
    ...
    // Default values used in learning cats & dogs model by Jason Brownlee
	int WHC[3] = {224, 224, 3};
    ...
    // predict list of images with this model
	vector<string>::iterator it;
	int i = 0;
	for (it = images.begin(); it != images.end(); it++, i++) {
        cimg_library::CImg<unsigned char> img;
		
	    img = cimg_library::CImg<unsigned char>(images[i].c_str());
		...
		if (debug) {
			string tensor;
			tensor = fdeep::show_tensor(input);
			
			// replace ], by ],\n for readability
			size_t index = 0;
			while (true) {
				/* Locate the substring to replace. */
				index = tensor.find("],", index);
				if (index == std::string::npos) break;

				/* Make the replacement. */
				tensor.replace(index, 3, "],\n");
        ...
        float fresult= model.predict_class_with_confidence({input}).second;
        cout << *it << " belongs to category " << setprecision(8) << fixed << fresult << endl;
    }
